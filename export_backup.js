...
// Função simplificada para criar contornos de grid
function createSimpleGridContours(points, gridBoxes, audioState) {
    console.log(`[createSimpleGridContours] 🚀 VERSÃO SIMPLIFICADA - ${points.length} pontos, ${gridBoxes.length} gridBoxes`);
    ...
    console.log(`[createSimpleGridContours] ✅ Criados ${contours.length} contornos`);
    return contours;
}
    console.log(`[createGridContoursFromPoints] === DEPURAÇÃO COMPLETA DOS DADOS DO CANVAS ===`);
    ...
    console.log(`[createGridContoursFromPoints] === VERIFICAÇÃO DOS DADOS ORIGINAIS ===`);
    if (typeof window !== 'undefined' && window.letterPoints) {
        ...
        console.log(`[createGridContoursFromPoints] Distribuição no CANVAS:`, canvasSegmentStats);
    }
    ...
    console.log(`[createGridContoursFromPoints] Distribuição RECEBIDA:`, receivedSegmentStats);
    
    // COMPARAR: Se há diferenças entre canvas e dados recebidos
    if (typeof window !== 'undefined' && window.letterPoints) {
        console.log(`[createGridContoursFromPoints] === COMPARAÇÃO CANVAS vs RECEBIDO ===`);
        const canvasSegmentStats = {};
        window.letterPoints.filter(p => p.renderType === 'grid').forEach(point => {
            const segment = point.segment !== undefined ? point.segment : 'undefined';
            canvasSegmentStats[segment] = (canvasSegmentStats[segment] || 0) + 1;
        ...
        Object.keys(canvasSegmentStats).forEach(segment => {
            ...
            if (canvasCount !== receivedCount) {
                console.log(`[createGridContoursFromPoints] ⚠️ DIFERENÇA no segmento ${segment}: Canvas=${canvasCount}, Recebido=${receivedCount}`);
            } else {
                console.log(`[createGridContoursFromPoints] ✅ Segmento ${segment}: Canvas=${canvasCount}, Recebido=${receivedCount}`);
            }
        });
    }
    
    // TERCEIRA ANÁLISE: Debug detalhado dos primeiros pontos
    console.log(`[createGridContoursFromPoints] === ANÁLISE DOS PRIMEIROS 10 PONTOS ===`);
    ...
    console.log(`[createGridContoursFromPoints] Pontos únicos preservados: ${uniquePoints.length}`);
    ...
    console.log(`[createGridContoursFromPoints] Distribuição FINAL após filtragem:`, finalSegmentStats);
    ...
    console.log(`[createGridContoursFromPoints] ✅ Criados ${gridBoxes.length} quadrados da grelha (${gridBoxes.length * 4} linhas de borda)`);

    // ANÁLISE DOS SEGMENTOS ANTES DA CRIAÇÃO
    const segmentAnalysis = {};
    uniquePoints.forEach(point => {
        const segment = point.segment !== undefined ? point.segment : 0;
        segmentAnalysis[segment] = (segmentAnalysis[segment] || 0) + 1;
    });
    console.log(`[createGridContoursFromPoints] 📊 Análise de segmentos antes da criação:`, segmentAnalysis);

    // 2. CRIAR PARTÍCULAS PRESERVANDO POSIÇÃO RELATIVA DENTRO DE CADA QUADRADO
    uniquePoints.forEach((point, pointIndex) => {
        const segment = point.segment !== undefined ? point.segment : 0;
        
        console.log(`[createGridContoursFromPoints] Partícula ${pointIndex}: segment=${segment}, x=${point.actualX.toFixed(1)}, y=${point.actualY.toFixed(1)}`);
        ...
        if (!containingBox) {
            console.log(`[createGridContoursFromPoints] ⚠️ Partícula ${pointIndex} pos(${point.actualX.toFixed(1)}, ${point.actualY.toFixed(1)}) não está dentro de nenhum quadrado, pulando`);
            return;
        }
        
        console.log(`[createGridContoursFromPoints] Partícula ${pointIndex} encontrada no quadrado ${boxIndex}: box(${containingBox.x.toFixed(1)}, ${containingBox.y.toFixed(1)}, ${containingBox.width.toFixed(1)}x${containingBox.height.toFixed(1)})`);
        ...
        console.log(`[createGridContoursFromPoints] Partícula ${pointIndex} no quadrado ${boxIndex}: rel(${relativeX.toFixed(2)}, ${relativeY.toFixed(2)}) -> font(${x.toFixed(1)}, ${y.toFixed(1)})`);
        
        // CAPTURAR VALORES VISUAIS EXATOS DO CANVAS NO MOMENTO DA EXPORTAÇÃO
        // Tentar obter valores salvos durante a renderização ou usar fallbacks
        let reactions = {
        ...
        // Criar contornos específicos por tipo de segmento - COM REAÇÕES DE ÁUDIO
        switch (segment) {
            case 0: // FLOCKING - círculos pequenos que reagem
                ...
                console.log(`[createGridContoursFromPoints] Criado círculo FLOCKING: radius=${particleContour.radius.toFixed(2)}, opacity=${reactions.opacity?.toFixed(2)}, aplicada=${particleContour.opacity?.toFixed(2)}`);
                break;
                
            case 1: // BASS WAVES - círculos que pulsam com o bass
                ...
                console.log(`[createGridContoursFromPoints] Criado círculo BASS: radius=${particleContour.radius.toFixed(2)}, glow=${reactions.glow?.toFixed(2)}, opacity=${particleContour.opacity?.toFixed(2)}`);
                break;
                
            case 2: // MID WAVES - círculos médios com movimento
                ...
                console.log(`[createGridContoursFromPoints] Criado círculo MID: radius=${particleContour.radius.toFixed(2)}, opacity=${particleContour.opacity?.toFixed(2)}`);
                break;
                
            case 3: // TREBLE LINES - linhas que reagem ao treble COM ROTAÇÃO
                ...
                // Usar rotação da partícula original (angle) ou criar uma baseada na posição
                const lineAngle = point.angle || (Math.atan2(finalY - 400, finalX - 400)); // Ângulo da linha
                
                // Calcular pontos da linha com rotação
                ...
                particleContour = {
                    ...
                    opacity: reactions.opacity, // Aplicar opacidade diretamente
                    audioReactions: reactions,
                    rotation: lineAngle, // Adicionar rotação aos metadados
                    metadata: { 
                        type: 'treble_line', 
                        pointIndex: pointIndex,
                        opacity: reactions.opacity,
            ...
            case 4: // ORGANIC - formas orgânicas que crescem com o áudio
                ...
                console.log(`[createGridContoursFromPoints] Criado círculo ORGANIC: radius=${particleContour.radius.toFixed(2)}, glow=${reactions.glow?.toFixed(2)}`);
                break;
                
            default:
                console.log(`[createGridContoursFromPoints] ⚠️ Segmento desconhecido: ${segment}, usando círculo padrão`);
                particleContour = {
                    type: 'circle',
                    centerX: finalX,
                    centerY: finalY,
        ...
        if (particleContour) {
            ...
            console.log(`[createGridContoursFromPoints] ✅ Partícula ${pointIndex} adicionada: seg=${segment}, tipo=${particleContour.metadata.type}, pos(${finalX.toFixed(1)}, ${finalY.toFixed(1)})`);
        } else {
            console.log(`[createGridContoursFromPoints] ❌ ERRO: Não foi possível criar contorno para partícula ${pointIndex}, segmento ${segment}`);
        }
    });
    ...
    console.log(`[createGridContoursFromPoints] ${numQuadrados} quadrados (${linhasQuadrados} linhas) + ${particulas} partículas = ${contours.length} contornos`);
...
function captureCurrentAudioState() {
    try {
        ...
        console.warn('Erro ao capturar estado do áudio:', error);
        return {
            spectrum: [],
...
async function exportarFonteComoOTF() {
    ...
    if (exportState.isExporting) {
        console.warn("Exportação já em progresso!");
        return;
    }
    ...
    try {
        console.log("=== INÍCIO DO PROCESSO DE EXPORTAÇÃO ===");
        ...
        console.log("Estado do áudio capturado:", audioState ? "✅" : "❌");

        // Verificar se há pontos de letra visíveis
        console.log("=== VERIFICAÇÃO DOS PONTOS DE LETRA ===");
        console.log("window.letterPoints existe?", typeof window.letterPoints !== 'undefined');
        console.log("window.letterPoints é array?", Array.isArray(window.letterPoints));
        ...
        if (typeof window.letterPoints === 'undefined' || !window.letterPoints || window.letterPoints.length === 0) {
            console.error("❌ ERRO: Nenhuma letra visível no canvas!");
            throw new Error("Nenhuma letra visível no canvas. Digite algo primeiro!");
        }

        console.log("✅ Pontos de letra encontrados:", window.letterPoints.length);
        ...
        console.log("=== SEPARAÇÃO POR TIPO DE RENDERIZAÇÃO ===");
        ...
        if (gridLetterGroups.size > 0 && vectorLetterGroups.size === 0) {
            ...
            console.error('[exportarFonteComoOTF] Nenhuma letra válida encontrada!');
            console.error('[exportarFonteComoOTF] Pontos disponíveis:', window.letterPoints);
...
async function createVectorGlyphFromPoints(char, charPoints, audioState = null) {
    ...
    const descendentesMaiusculas = 'QJ'; // Q sempre, J às vezes dependendo do design
    ...
    if (hasDescendente) {
        ...
        if (isUpper && descendentesMaiusculas.includes(char)) {
            // Para Q e J maiúsculos: offset menor (cauda mais curta que minúsculas)
            if (char === 'Q') {
...
function createGridGlyphFromPoints(char, charPoints, audioState = null) {
    console.log(`🚀 FUNÇÃO REAL CHAMADA: createGridGlyphFromPoints para '${char}'`);
    ...
    if (points.length === 0 && charPoints) {
        ...
        console.log(`[createGridGlyphFromPoints] ⚠️ Usando dados dos parâmetros: ${points.length} pontos`);
    }
    
    console.log(`[createGridGlyphFromPoints] 🎯 CHECKPOINT A: ${points.length} pontos, ${gridBoxes.length} caixas antes dos fallbacks`);
    ...
    console.log(`[createGridGlyphFromPoints] 🎯 CHECKPOINT B: ${points.length} pontos, ${gridBoxes.length} caixas após fallbacks`);
    ...
    if ((!gridBoxes || gridBoxes.length === 0) && points.length > 0) {
        console.log(`[createGridGlyphFromPoints] 🔄 Criando gridBoxes a partir dos pontos...`);
        ...
        console.log(`[createGridGlyphFromPoints] ✅ Criados ${gridBoxes.length} gridBoxes`);
    }

    console.log(`[createGridGlyphFromPoints] 🎯 CHECKPOINT C: ${points.length} pontos, ${gridBoxes.length} caixas finais`);

    // INTERCEPTAR AQUI E USAR NOVA LÓGICA
    console.log(`[createGridGlyphFromPoints] 🚀 INTERCEPTAÇÃO: Redirecionando para createGridContoursFromPoints`);
    
    try {
        const newContours = createSimpleGridContours(points, gridBoxes, audioState);
        console.log(`[createGridGlyphFromPoints] ✅ NOVA LÓGICA: Recebidos ${newContours ? newContours.length : 0} contornos`);
        ...
        if (newContours && newContours.length > 0) {
            ...
            console.log(`[createGridGlyphFromPoints] ✅ RETORNANDO (INTERCEPTADO): ${result.contours.length} contornos`);
            return result;
        }
    } catch (error) {
        console.error(`[createGridGlyphFromPoints] ❌ Erro na interceptação:`, error);
    }
    
    console.log(`[createGridGlyphFromPoints] ⚠️ Interceptação falhou, continuando com lógica antiga`);
    ...
    if ((points.length === 0) && (gridBoxes.length === 0)) {
        console.log(`[createGridGlyphFromPoints] 🚨 FALLBACK VAZIO: Criando quadrado central`);
        const advanceWidth = calculateProportionalAdvanceWidth(char);
    ...
    if (audioState && points.length > 0) {
        ...
        gridBoxes.forEach((box, boxIndex) => {
            ...
            for (const [segment, segmentPoints] of boxSegmentGroups) {
                ...
                // 🎨 PADRÕES VISUAIS FIXOS POR SEGMENTO (sem copiar reações do canvas)
                console.log(`[createGridContoursFromPoints] 🎯 Box ${boxIndex} | Segment ${segment} | Aplicando padrão fixo do segmento`);

                // SEGMENTO 0: FLOCKING → 1 ponto pequeno no centro
                if (segment === 0 && originalPoint) {
                    const pointSize = 2; // Pequeno e fixo
                    
                    const circleResult = createCircleContour(boxCenterX, boxCenterY, pointSize, 0.1); // Força opacidade baixa
                ...
                // SEGMENTO 1: BASS → 4 pontos em cruz
                else if (segment === 1 && originalPoint) {
                    const pointSize = 3; // Médio
                    const offset = 4; // Distância do centro
                    ...
                    positions.forEach(pos => {
                        const circleResult = createCircleContour(pos.x, pos.y, pointSize, 0.3); // Força opacidade baixa
...
function createCircleContour(x, y, size, opacity = 1.0) {
    console.log(`[createCircleContour] 📊 Criando círculo: pos=(${x.toFixed(1)}, ${y.toFixed(1)}), size=${size.toFixed(1)}, opacity=${opacity.toFixed(3)}`);
    
    // Simular opacidade através de diferentes densidades/padrões
    if (opacity <= 0.2) {
        // Opacidade muito baixa: apenas um ponto pequeno no centro
        console.log(`[createCircleContour] 🔸 OPACIDADE MUITO BAIXA: ponto pequeno`);
        return {
            type: 'circle',
            centerX: x,
            centerY: y,
        ...
        console.log(`[createCircleContour] 🔸 OPACIDADE BAIXA: 4 pontos em cruz`);
        const contours = [];
        ...
        // Opacidade média: círculo com 60% do tamanho
        console.log(`[createCircleContour] 🔸 OPACIDADE MÉDIA: círculo 60%`);
        return {
            type: 'circle',
            centerX: x,
            centerY: y,
        ...
        // Opacidade alta: círculo com 80% do tamanho
        console.log(`[createCircleContour] 🔸 OPACIDADE ALTA: círculo 80%`);
        return {
            type: 'circle',
            centerX: x,
            centerY: y,
        ...
        // Opacidade máxima: círculo completo
        console.log(`[createCircleContour] 🔴 OPACIDADE MÁXIMA: círculo completo`);
        return {
            type: 'circle',
            centerX: x,
            centerY: y,
...
function addContourToPath(path, contour) {
    ...
    if (contour.type === 'circle') {
        ...
        // Criar círculo outline - dois círculos concêntricos
        const outerR = contour.radius;
        const innerR = Math.max(0, outerR - contour.strokeWidth);
        const x = contour.centerX;
        const y = contour.centerY;

        // Círculo externo (sentido horário)
        ...
        // Círculo interno (sentido anti-horário para criar "buraco")
        if (innerR > 0) {
...
async function convertToOpenType(fontData) {
    ...
    if (typeof opentype === 'undefined') {
        console.error("Biblioteca opentype.js não encontrada!");
    ...
    try {
        ...
        console.error("Erro na conversão OpenType:", error);
        throw error;
    }
}
...
if (typeof window !== 'undefined') {
    ...
    window.testarExportacaoSimples = async function () {
        try {
            ...
            console.error('🧪 TESTE SIMPLES: Stack trace:', error.stack);
        }
    };
    ...
    window.testarExportacaoGrelhaEspecifica = async function () {
        try {
            ...
            console.error('🧪 TESTE GRELHA ESPECÍFICA: Erro:', error);
            console.error('🧪 TESTE GRELHA ESPECÍFICA: Stack trace:', error.stack);
        }
    };
    ...
    window.testarGlyphIndividual = function () {
        try {
            ...
            if (glyph && glyph.contours && glyph.contours.length > 0) {
                ...
                console.error('🧪 TESTE GLYPH INDIVIDUAL: ❌ Glyph inválido!');
            }

        } catch (error) {
            console.error('🧪 TESTE GLYPH INDIVIDUAL: Erro:', error);
            console.error('🧪 TESTE GLYPH INDIVIDUAL: Stack trace:', error.stack);
        }
    };
    ...
    window.forcarDetecaoGrelha = function () {
        if (!window.letterPoints || window.letterPoints.length === 0) {
            console.error('🔧 FORÇANDO: Nenhum ponto disponível!');
            return;
        }
    ...
    window.verificarDetecao = function () {
        if (!window.letterPoints || window.letterPoints.length === 0) {
            console.error('🔍 VERIFICANDO: Nenhum ponto disponível!');
            return;
        }
    ...
    window.testarDownload = async function () {
        try {
            ...
            console.error('📥 TESTE: Erro no download:', error);
        }
    };
}
...
async function exportarFonteGrelhaSeparada(gridLetterGroups = null, audioState = null) {
    ...
    try {
        ...
        if (finalGridLetterGroups.size === 0) {
            console.error('[exportarFonteGrelhaSeparada] ❌ Nenhuma letra grelha encontrada!');
            console.error('[exportarFonteGrelhaSeparada] Pontos disponíveis:', window.letterPoints);
        ...
        console.error("Erro durante exportação grelha:", error);
        exportState.errors.push(error.message);
        throw error;
    } finally {
        // Restaurar visibilidade original do segmento 0
...
async function createFontFromGridLetters(letterGroups, audioState = null, referenceName = "") {
    console.log(`[createFontFromGridLetters] === CRIAÇÃO DE FONTE GRELHA ===`);
    console.log(`[createFontFromGridLetters] Referência: "${referenceName}"`);
    console.log(`[createFontFromGridLetters] Caracteres disponíveis:`, Array.from(letterGroups.keys()));
    ...
    for (const char of availableLetters) {
        try {
            ...
            if (glyphData) {
                glyphs.push(glyphData);
                console.log(`[createFontFromGridLetters] ✅ Glyph criado para '${char}'`);
            } else {
                console.warn('[createFontFromGridLetters] ❌ GlyphData é null para:', char);
            }
        } catch (error) {
            console.error(`❌ Erro ao criar glyph grelha para '${char}':`, error);
...
async function exportarSegmentosComReacoes() {
    ...
    try {
        ...
        if (!audioState.spectrum || audioState.spectrum.length === 0) {
            console.warn("Estado do áudio não disponível, usando valores padrão");
        ...
        console.log(`✅ Fonte exportada com reações de segmento: ${filename}`);
        console.log(`📊 Estado do áudio capturado:`, audioState);

    } catch (error) {
        console.error("Erro durante exportação de segmentos reativos:", error);
        exportState.errors.push(error.message);
        throw error;
    } finally {
        exportState.isExporting = false;
